...
import { executeTheSportsDbWriteSync } from "../../server/lib/resultSync.ts";
...
const authorize = (event: NetlifyEvent, body: Record<string, any>) => {
  ...
  if (!providedSecret || providedSecret !== expectedSecret) {
    return { ok: false, statusCode: 401, body: { error: "Unauthorized result sync request." } };
  }
...
export const handler = async (event: NetlifyEvent) => {
  ...
  try {
    ...
    if (provider !== "thesportsdb") {
      return jsonResponse(400, { error: `Unsupported write provider: ${provider || "missing"}` });
    }
    ...
    console.error("TheSportsDB result sync function error:", err);
    return jsonResponse(500, {
      error: "Chyba pri TheSportsDB result syncu: " + err.message,
      mode: "write",
