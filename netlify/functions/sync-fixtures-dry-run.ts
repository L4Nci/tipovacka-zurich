...
import { buildTheSportsDbFixtureDryRunResponse } from "../../server/lib/fixtureSync.ts";
...
const authorize = (event: NetlifyEvent, body: Record<string, any>) => {
  ...
  if (!providedSecret || providedSecret !== expectedSecret) {
    return { ok: false, statusCode: 401, body: { error: "Unauthorized fixture dry-run request." } };
  }
...
export const handler = async (event: NetlifyEvent) => {
  ...
  try {
    ...
    if (provider !== "thesportsdb") {
      return jsonResponse(400, { error: `Unsupported fixture dry-run provider: ${provider || "missing"}` });
    }
    ...
    console.error("TheSportsDB fixture dry-run function error:", err);
    return jsonResponse(500, {
      error: "Chyba pri TheSportsDB fixture dry-runu: " + err.message,
      mode: "dry_run",
