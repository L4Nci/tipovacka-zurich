...
import { buildTheSportsDbDryRunResponse } from "../../server/lib/resultSync.ts";
...
const authorize = (event: NetlifyEvent, body: Record<string, any>) => {
  ...
  if (!providedSecret || providedSecret !== expectedSecret) {
    return { ok: false, statusCode: 401, body: { error: "Unauthorized dry-run request." } };
  }
...
export const handler = async (event: NetlifyEvent) => {
  ...
  try {
    const provider = String(event.queryStringParameters?.provider || body.provider || "api-football");
    if (provider !== "thesportsdb") {
      return jsonResponse(400, { error: `Unsupported dry-run provider: ${provider}` });
    }
    ...
    console.error("TheSportsDB dry-run function error:", err);
    return jsonResponse(500, {
      error: "Chyba pri TheSportsDB dry-runu: " + err.message,
      dry_run: true,
      wrote_to_db: false
    });
  }
